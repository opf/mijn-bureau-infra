...
deny contains msg if {
  ...
  msg := sprintf("initContainers '%s' in Deployment '%s' has no resource requests or limits defined - global.resourcesPreset may not be properly propagating", [container.name, input.metadata.name])
}
...
deny contains msg if {
  ...
  not container.resources.requests
  not container.resources.limits
  msg := sprintf("initContainers '%s' in StatefulSet '%s' has no resource requests or limits defined - global.resourcesPreset may not be properly propagating", [container.name, input.metadata.name])
}
...
deny contains msg if {
  ...
  msg := sprintf("initContainers '%s' in '%s' '%s' has empty CPU request - global.resourcesPreset may not be applied correctly", [container.name, input.kind, input.metadata.name])
}
...
deny contains msg if {
  ...
  msg := sprintf("initContainers '%s' in '%s' '%s' has empty memory request - global.resourcesPreset may not be applied correctly", [container.name, input.kind, input.metadata.name])
}

...
deny contains msg if {
  ...
  msg := sprintf("Container '%s' in '%s' '%s' has memory request '%s' but xlarge preset should set '3072Mi' (propagation failed)", [container.name, input.kind, input.metadata.name, memory_request])
}
...
deny contains msg if {
  ...
  msg := sprintf("Container '%s' in '%s' '%s' has CPU limit '%s' but xlarge preset should set '3.0' (propagation failed)", [container.name, input.kind, input.metadata.name, cpu_limit])
}
...
deny contains msg if {
  ...
  msg := sprintf("Container '%s' in '%s' '%s' has memory limit '%s' but xlarge preset should set '6144Mi' (propagation failed)", [container.name, input.kind, input.metadata.name, memory_limit])
}
...
deny contains msg if {
  ...
  msg := sprintf("Container '%s' in '%s' '%s' has ephemeral-storage request '%s' but xlarge preset should set '50Mi' (propagation failed)", [container.name, input.kind, input.metadata.name, ephemeral_request])
}
...
deny contains msg if {
  ...
  msg := sprintf("Container '%s' in '%s' '%s' has ephemeral-storage limit '%s' but xlarge preset should set '2Gi' (propagation failed)", [container.name, input.kind, input.metadata.name, ephemeral_limit])
}
